...
def draw():
    ...
    cv.rect(5, 8, 46, 43, SHADOW)
    # window: 1px black outline, light title bar, dark screen
    cv.rect(2, 5, 43, 40, BLACK)
    cv.rect(3, 6, 42, 12, BAR)
    cv.rect(3, 13, 42, 39, SCREEN)
    cv.rect(3, 12, 42, 12, BLACK)                 # rule under the title bar
    cv.rect(5, 8, 8, 10, BARDK)                   # miniaturise button (left)
    cv.frame(5, 8, 8, 10, BLACK)
    cv.rect(37, 8, 40, 10, BARDK)                 # close button (right)
    cv.frame(37, 8, 40, 10, BLACK)
    # prompt:  >_   then two dim "output" lines
    cv.bitmap(6, 16, ["#...",
                      ".#..",
                      "..#.",
                      "...#",
                      ...
                      ".#..",
                      "#..."], GREEN)
    cv.rect(13, 22, 18, 23, GREEN)                # underscore cursor
    cv.rect(6, 26, 20, 26, DIMGRN)
    cv.rect(6, 29, 13, 29, DIMGRN)
    cv.rect(6, 32, 17, 32, DIMGRN)
    # padlock, lower right: shackle, body, keyhole
    cv.rect(31, 22, 38, 23, STEEL)                # top of the shackle
    cv.rect(31, 22, 32, 30, STEEL)
    cv.rect(37, 22, 38, 30, STEEL)
    cv.rect(28, 29, 41, 39, BLACK)                # body outline
    cv.rect(29, 30, 40, 38, GOLD)
    cv.rect(29, 37, 40, 38, GOLDDK)
    cv.rect(34, 32, 35, 35, BLACK)                # keyhole
    return cv
